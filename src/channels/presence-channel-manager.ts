...
export class PresenceChannelManager extends PrivateChannelManager {
    ...
    join(
        ...
        return this.server.adapter
            .getChannelMembersCount(app.id, channel)
            .then((membersCount) => {
                if (membersCount + 1 > app.maxPresenceMembersPerChannel) {
                    return {
                        success: false,
                        ws,
                        errorCode: 4100,
                ...
                let member: PresenceMember = JSON.parse(
                    message.data.channel_data,
                );

                let memberSizeInKb = Utils.dataToKilobytes(member.user_info);

                if (memberSizeInKb > app.maxPresenceMemberSizeInKb) {
                    return {
                        success: false,
                        ws,
                        errorCode: 4301,
                        errorMessage: `The maximum size for a channel member is ${app.maxPresenceMemberSizeInKb} KB.`,
                        type: "LimitReached",
                    };
                }
    ...
    leave(ws: WebSocket<unknown>, channel: string): Promise<LeaveResponse> {
        return super.leave(ws, channel).then((response) => {
            return {
                ...response,
                ...{
                    member: (ws as any).presence.get(channel),
                },
            };
        });
    }
