...
export class PrometheusMetricsDriver implements MetricsInterface {
    ...
    markNewConnection(ws: WebSocket<unknown>): void {
        ...
        this.metrics.connectedSockets.inc(this.getTags(app.id));
        this.metrics.newConnectionsTotal.inc(this.getTags(app.id));
    }
    ...
    markDisconnection(ws: WebSocket<unknown>): void {
        const app = (ws as any).app;
        this.metrics.connectedSockets.dec(this.getTags(app.id));
        this.metrics.newDisconnectionsTotal.inc(this.getTags(app.id));
    }
    ...
    trackHorizontalAdapterResolvedPromises(
        ...
        if (resolved) {
            this.metrics.horizontalAdapterResolvedPromises.inc(
                this.getTags(appId),
            );
        } else {
            this.metrics.horizontalAdapterUncompletePromises.inc(
                this.getTags(appId),
            );
        }
    }
    ...
    markHorizontalAdapterResponseReceived(appId: string): void {
        this.metrics.horizontalAdapterReceivedResponses.inc(
            this.getTags(appId),
        );
    }
